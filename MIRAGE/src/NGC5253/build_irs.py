...
##----------------------------------------------------------
pre_calib1 = input("Run inter-calibration prepipeline (LVL) (y/n)? ")
...
for i in range(Nphot):
    ...
    raw_phot1 = path_phot+src+'_'+phot[i]+'_LVL'
    if phot[i]=='IRAC4':
        wgt_phot1 = raw_phot1+'_cov'
    elif phot[i]=='MIPS1':
        wgt_phot1 = raw_phot1+'_wt'
    fits_phot1 = path_cal+src+'_'+phot[i]+'_LVL'
    tmp_phot1 = path_tmp+'calib/'+src+'_'+phot[i]+'_LVL'
    ...
    if pre_calib1=='y':
        ...
        ## (Sect. 2.1.2) The coverage maps each contain effective exposure times
        ## calculated on a pixel-by-pixel basis. These maps are in units of seconds
        ## and named using the standard IRAC convention with an additional identifier
        ## concatenated to the end.
        ## Similarly to IRAC, weight maps are delivered as well; however,
        ## unlike the IRAC coverage maps,the values of the pixels in the MIPS
        ## weight maps store the number of images stacked at that point in the map.
        ## (Sect. 4.2) The EXPTIME keyword indicates the total exposure time,
        ## in seconds, for an average pixel in the center of the map
        ## (i.e., the exposure time for any pixel is equal to
        ## EXPTIME * weight map / <average value in center of weight map>).
        ## -- LVL DR5 doc
        ## https://irsa.ipac.caltech.edu/data/SPITZER/LVL/LVL_DR5_v5.pdf
        
        ## The coverage map give the sum of the weighted detector response
        ## at each image pixel over all detector samples. Here we can use
        ## IRAC coverage maps as weight maps by equivalently adopting wfac
        ## via header keyword "MAXCOV": wfac = MAXCOV / max_val_of_cov_map

        ## Only need to run ONCE for each photometry (comment after using)
        # gen_unc2 = input("Create LVL IRAC4/MIPS1 uncertainty map (y/n)? ")
        # if gen_unc2=='y':
        #     ds_wgt = read_fits(wgt_phot1)
        #     if phot[i]== 'IRAC4':
        #         bg = read_fits(raw_phot1).data[360:400,660:700]
        #         bg_wgt = ds_wgt.data[360:400,660:700]
        #         wfac = ds_wgt.header['MAXCOV'] / max(ds_wgt.data.flatten())
        #     elif phot[i]=='MIPS1':
        #         bg = read_fits(raw_phot1).data[1200:1240,680:720]
        #         bg_wgt = ds_wgt.data[1200:1240,680:720]
        #         wfac = .1
        #     iuncert(raw_phot1, filOUT=raw_phot1+'_unc',
        #             filWGT=wgt_phot1, wfac=wfac,
        ...
        mtg.reproject_mc(raw_phot1, refheader=coadd_fp_crop,
                         dist='norm', Nmc=Nmc, filOUT=tmp_phot1)
        
        ## Convolve phot
        for j in trange(Nmc+1, leave=False,
                        desc=phot[i]+': LVL conv [MC]'):
            if j==0:
    ...
    if int_calib=='y':
        ...
        p0 = pplot(pix_phot1, pix_spec,
                   ...
                   xlab='LVL (MJy/sr)', ylab='IRS (MJy/sr)',
                   figsize=(8,8), title=src+'_'+phot[i])
        
        ## Linear fit
        if i==0:
            mask_mips1 = pix_phot1>0
