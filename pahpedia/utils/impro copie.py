...
def swarpj(filIN, filOUT, REFile):
	...
	SP.call('swarp '+filIN+'.fits -IMAGEOUT_NAME '+filOUT+'.fits \
		-RESAMPLING_TYPE BILINEAR -SUBTRACT_BACK N \
		-CENTER_TYPE MANUAL,MANUAL -CENTER '+CRVAL1+','+CRVAL2,
		shell=True)

	data, footprint = reproject_interp(filOUT+'.fits', hdr)
...
def crop(filIN, filOUT, centre, size, nopr=False):
	...
	xcl = math.floor(xc) # pixel center left
	ycl = math.floor(yc) # pixel center lower
	...
	## lowerleft centre
	xmin = xcl - math.floor(size[1]/2.)
	xmax = xmin + size[1]
	ymin = ycl - math.floor(size[0]/2.)
	ymax = ymin + size[0]
	if not (xmin>=0 and xmax<=NAXIS1 and ymin>=0 and ymax<=NAXIS2):
		print("Error: crop region overpassed image border! ")
		exit()
	## modify header
	xc0 = xmin + size[1]/2. # pixel center center
	yc0 = ymin + size[0]/2. # pixel center center
