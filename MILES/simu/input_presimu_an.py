...
##
##-----------------------------
input_fit = croot+'input_presimu_bb.py'
...
t_end = []
## BB
bb_log = pathlib.Path(dirout+'parlog_presimu_bb'+h5ext)
if bb_log.exists():
    Nmcmc = read_hdf5(dirout+'parlog_presimu_bb', 'Last index')[0]
else:
    Nmcmc = read_hdf5(dirin+'input_master', 'Nmcmc')[0]
t_burnin.append( int(Nmcmc * 0.3)-1 )
t_end.append( Nmcmc )
## HB
hb_log = pathlib.Path(dirout+'parlog_presimu_hb'+h5ext)
if hb_log.exists():
    Nmcmc = read_hdf5(dirout+'parlog_presimu_hb', 'Last index')[0]
else:
    Nmcmc = read_hdf5(dirin+'input_master', 'Nmcmc')[0]
t_burnin.append( int(Nmcmc * 0.3)-1 )
t_end.append( Nmcmc )
