...
for m in mode:
    ...
    if h5_out.exists():
        ...
        for j in range(int(Npar/9)+1):
            filename = path_fig+'par_'+m+'_ipar='+str(j*9+1)+'-'+str(j*9+9)+'.png'
        ...
        ##-------------------------
        for j in range(Ncal):
            filename = path_fig+'par_'+m+'_cal_'+instr[j]+'.png'
            ...
            p = pplot(np.exp(ln1pdmcmc[:Nmcmc,j,y,x]), counter,
                      ...
                      xlabel='Calib factor', ylabel='MCMC counter',
