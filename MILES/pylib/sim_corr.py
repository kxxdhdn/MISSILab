...
## Point color distinguishes spectra
clist = [#'k',
         'c',
         'g',
         'm',
         'orange']
## Point shape distinguishes fitting methods
mlist = ['o', # sim
         's', # chi2
         '^', # bb
         '*',] # hb
        ...
        for i in range(Ncorr):
            ...
            for q in range(Nq):
                ...
                p.add_plot(Rsim[x,q,:], Rsim[y,q,:],
                           marker=mlist[0], markersize=10, mfc='none',
                           fmt='s', c=clist[q], label='sim_'+labelist[q],)
                p.add_plot(Rfit[x,q,:], Rfit[y,q,:], yerr=Rerr[y,q,:], xerr=Rerr[x,q,:],
                           fmt='s', ec='grey', zorder=100,
                           marker=mlist[im], markersize=10, capsize=2,
                           c=clist[q], label='fit_'+labelist[q])
