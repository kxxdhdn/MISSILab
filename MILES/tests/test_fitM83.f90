...
PROGRAM test_fitM83
  ...
  USE fitM83_external, ONLY: funcResid, Nx, Ny, NwOBS, wOBS, nuOBS, &
  ...
  !   chi2fity: DO yOBS=1,Ny
      notmasked: IF (ALL( mask(xOBS,yOBS,:) )) THEN
        ...
        CALL chi2min_LM (funcResid, NwOBS, parr(xOBS,yOBS,:), tol, &
                         STATUS=status(xOBS,yOBS), VERBOSE=.True., &
