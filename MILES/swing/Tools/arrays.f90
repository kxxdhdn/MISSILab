...
MODULE arrays
  ...
  INTERFACE reverse
    MODULE PROCEDURE reverse_DP, reverse_int ,reverse_char
...
CONTAINS
  ...
  PURE FUNCTION reverse_DP (x)
    
    USE utilities, ONLY: DP
    IMPLICIT NONE

    REAL(DP), INTENT(IN), DIMENSION(:) :: x
    REAL(DP), DIMENSION(SIZE(x)) :: reverse_DP
  ...
  END FUNCTION reverse_DP

  !-------------------------------------------------------------------------

  PURE FUNCTION reverse_int (x)
    
    IMPLICIT NONE

    INTEGER, INTENT(IN), DIMENSION(:) :: x
    INTEGER, DIMENSION(SIZE(x)) :: reverse_int
  ...
  PURE FUNCTION reverse_char (x)
    
    IMPLICIT NONE

    CHARACTER(*), INTENT(IN), DIMENSION(:) :: x
    CHARACTER(LEN(x)), DIMENSION(SIZE(x)) :: reverse_char
