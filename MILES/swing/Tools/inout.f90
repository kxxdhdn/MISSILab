...
MODULE inout
  ...
  PUBLIC :: write_ascii, read_ascii, write_binary, read_binary, readcol
  ...
  INTEGER, PARAMETER :: rank0 = 7 ! rank of dataset
...
CONTAINS
  ...
  !=========================================================================
  ! CALL WRITE_HDF5(DBLARR{1-7}D or STRARR{1-2}D or INTARR{1-3}D,FILE="", &
  ...
  !                 IND6=[idim6_inf,idim6_sup],IND7=[idim7_inf,idim7_sup],UNIT=)
  ! 
  !   Write a 1D to 7D double precision array or a 1D string array to an 
  ...
  SUBROUTINE write_hdf5 (dblarr1D,dblarr2D,dblarr3D,dblarr4D,dblarr5D, &
                          dblarr6D,dblarr7D, &
                          strarr1D,strarr2D, &
                          intarr1D,intarr2D,intarr3D,initdblarr, &
                          initintarr, &
                          file,name,compress,append,ind1,ind2,ind3,ind4,ind5, &
                          ind6,ind7,unit,verbose)
    ...
    REAL(DP), DIMENSION(:,:,:,:,:,:,:), INTENT(IN), OPTIONAL :: dblarr7D
    CHARACTER(*), DIMENSION(:), INTENT(IN), OPTIONAL :: strarr1D
    CHARACTER(*), DIMENSION(:,:), INTENT(IN), OPTIONAL :: strarr2D
    ...
    INTEGER, DIMENSION(2), INTENT(IN), OPTIONAL :: ind6, ind7
    ...
    INTEGER(hsize_t), DIMENSION(2) :: i1, i2, i3, i4, i5, i6, i7 ! minmax indices
    ...
    IF (PRESENT(dblarr6D)) THEN
      ...
      END IF
    END IF
    IF (PRESENT(dblarr7D)) THEN
      IF (rank == -1) THEN
        rank = 7
        dims(1:rank) = [ SIZE(dblarr7D(:,:,:,:,:,:,:),1), &
                         SIZE(dblarr7D(:,:,:,:,:,:,:),2), &
                         SIZE(dblarr7D(:,:,:,:,:,:,:),3), &
                         SIZE(dblarr7D(:,:,:,:,:,:,:),4), &
                         SIZE(dblarr7D(:,:,:,:,:,:,:),5), &
    ...
    IF (PRESENT(strarr1D)) THEN
      ...
      END IF
    END IF
    IF (PRESENT(strarr2D)) THEN
      IF (rank == -1) THEN
        rank = 2
        dims(1:rank) = [ SIZE(strarr2D(:,:),1), SIZE(strarr2D(:,:),2) ]
        string = .True.
        strlen = LEN(strarr2D(:,:))
      ELSE
        CALL STRIKE("WRITE_HDF5","you can select only one ARRAY shape")
    ...
    IF (PRESENT(ind6) .AND. rank < 6) &
      CALL STRIKE("WRITE_HDF5", &
                  "you cannot select IND6 if ARRAY is less than 6D")
    IF (PRESENT(ind7) .AND. rank < 7) &
      CALL STRIKE("WRITE_HDF5", &
                  "you cannot select IND7 if ARRAY is less than 7D")
    IF (appnd .AND. subset) &
    ...
    fullorpartial: IF (.NOT. subset) THEN
      ...
      IF (.NOT. init) THEN
        writefull: SELECT CASE (rank)
          ...
          CASE (2)
            ...
            ELSE
              CALL H5DWRITE_F(DSET_ID=dset_ID,MEM_TYPE_ID=dtype_ID, &
                              BUF=strarr2D(:,:),DIMS=dims(1:rank),HDFERR=error)
            END IF
          CASE (3)
            IF (.NOT. integ) THEN
              CALL H5DWRITE_F(DSET_ID=dset_ID,MEM_TYPE_ID=dtype_ID, &
      ...
      ELSE
        writeinit: SELECT CASE (rank)
          ...
          CASE (7)
            CALL H5DWRITE_F(DSET_ID=dset_ID,MEM_TYPE_ID=dtype_ID, &
                            BUF=[0._DP,0._DP,0._DP,0._DP,0._DP,0._DP,0._DP], &
                            DIMS=dims0(1:rank),HDFERR=error)
    ...
    ELSE
      ...
      END IF
      IF (PRESENT(ind7)) THEN 
        i7(:) = ind7(:) 
      ELSE 
        i7(:) = [ INT(1,hsize_t), dims(7) ]
      END IF
      offset(:) = [ i1(1), i2(1), i3(1), i4(1), i5(1), i6(1), &
                    i7(1) ] - 1 ! offset starts at 0
      ...
      writesub: SELECT CASE (rank)
        ...
        CASE (2)
          ...
          ELSE
            CALL H5DWRITE_F(DSET_ID=dset_ID,MEM_TYPE_ID=dtype_ID, &
                            BUF=strarr2D(:,:),DIMS=hyperdims(1:rank), &
                            HDFERR=error,MEM_SPACE_ID=mspace_ID, &
                            FILE_SPACE_ID=dspace_ID)
          END IF
        CASE (3)
        ...
        CASE (7)
          CALL H5DWRITE_F(DSET_ID=dset_ID,MEM_TYPE_ID=dtype_ID, &
                          BUF=dblarr7D(:,:,:,:,:,:,:),DIMS=hyperdims(1:rank), &
                          HDFERR=error,MEM_SPACE_ID=mspace_ID, &
                          FILE_SPACE_ID=dspace_ID)
      END SELECT writesub
  ...
  !=========================================================================
  ! CALL READ_HDF5(DBLARR{1-7}D or STRARR{1-2}D or INTARR{1-3}D,FILE="",NAME="", 
  ...
  !                IND6=[idim6_inf,idim6_sup],IND7=[idim7_inf,idim7_sup], &
  ...
  SUBROUTINE read_hdf5 (dblarr1D,dblarr2D,dblarr3D,dblarr4D,dblarr5D,dblarr6D, &
                        dblarr7D,strarr1D,strarr2D,intarr1D,intarr2D,intarr3D, &
                        file,name,ind1,ind2,ind3,ind4,ind5,ind6,ind7,N1,N2,N3, &
                        N4,N5,N6,N7)
    ...
    CHARACTER(*), DIMENSION(:,:), ALLOCATABLE, INTENT(OUT), OPTIONAL :: strarr2D
    ...
    INTEGER, DIMENSION(2), INTENT(IN), OPTIONAL :: ind1, ind2, ind3, ind4, ind5
    INTEGER, DIMENSION(2), INTENT(IN), OPTIONAL :: ind6, ind7
    INTEGER, INTENT(OUT), OPTIONAL :: N1, N2, N3, N4, N5, N6, N7
    ...
    INTEGER(hsize_t), DIMENSION(rank0) :: offset   ! hyperslab offset
    INTEGER(hsize_t), DIMENSION(2) :: i1, i2, i3, i4 ,i5, i6, i7 ! [min,max] ind
    INTEGER :: rank, strlen
    INTEGER :: error ! error flag
    ...
    IF (PRESENT(ind6) .AND. rank < 6) &
      CALL STRIKE("READ_HDF5","you cannot select IND6 if ARRAY is less than 6D")
    IF (PRESENT(N6) .AND. rank < 6) &
      CALL STRIKE("READ_HDF5","you cannot select N6 if ARRAY is less than 6D")
    IF (PRESENT(ind7) .AND. rank < 7) &
      CALL STRIKE("READ_HDF5","you cannot select IND7 if ARRAY is less than 7D")
    IF (PRESENT(N7) .AND. rank < 7) &
      CALL STRIKE("READ_HDF5","you cannot select N7 if ARRAY is less than 7D")
      ...
      fullread: SELECT CASE (rank)
        ...
        CASE (2)
          IF (.NOT. string) THEN
            ...
            END IF
          ELSE
            ALLOCATE (strarr2D(dims(1),dims(2)))
            CALL H5DREAD_F(DSET_ID=dset_ID,MEM_TYPE_ID=dtype_ID, &
                           BUF=strarr2D(:,:),DIMS=dims(1:rank),HDFERR=error)
          END IF
        CASE (3)
        ...
        CASE (7)
          ALLOCATE (dblarr7D(dims(1),dims(2),dims(3),dims(4),dims(5),dims(6), &
                             dims(7)))
          CALL H5DREAD_F(DSET_ID=dset_ID,MEM_TYPE_ID=dtype_ID, &
                          BUF=dblarr7D(:,:,:,:,:,:,:),DIMS=dims(1:rank), &
                          HDFERR=error)
      ...
      subread: SELECT CASE (rank)
        ...
        CASE (2)
          IF (.NOT. string) THEN
            ...
            END IF
          ELSE
            ALLOCATE (strarr2D(hyperdims(1),hyperdims(2)))
            CALL H5DREAD_F(DSET_ID=dset_ID,MEM_TYPE_ID=dtype_ID, &
                           BUF=strarr2D(:,:),DIMS=hyperdims(1:rank), &
                           HDFERR=error,MEM_SPACE_ID=mspace_ID, &
                           FILE_SPACE_ID=dspace_ID)
        ...
        CASE (6)
          ...
          CALL H5DREAD_F(DSET_ID=dset_ID,MEM_TYPE_ID=dtype_ID, &
                         ...
                         FILE_SPACE_ID=dspace_ID)
        CASE (7)
          ALLOCATE (dblarr7D(hyperdims(1),hyperdims(2),hyperdims(3), &
                             hyperdims(4),hyperdims(5),hyperdims(6), &
                             hyperdims(7)))
          CALL H5DREAD_F(DSET_ID=dset_ID,MEM_TYPE_ID=dtype_ID, &
                         BUF=dblarr7D(:,:,:,:,:,:,:),DIMS=hyperdims(1:rank), &
    ...
    !---------------------------------------
    IF (subset) CALL H5SCLOSE_F(SPACE_ID=mspace_ID,HDFERR=error) ! close mem
    CALL H5SCLOSE_F(SPACE_ID=dspace_ID,HDFERR=error) ! close dataspace
    CALL H5DCLOSE_F(DSET_ID=dset_ID,HDFERR=error)    ! close dataset
    CALL H5FCLOSE_F(FILE_ID=file_ID,HDFERR=error)    ! close file access
    CALL H5CLOSE_F(ERROR=error)                      ! close Fortran interface
    ...
    IF (PRESENT(N7)) N7 = INT(dims(7),KIND(N7))
  ...
  !=========================================================================
  ! CALL GETDIM_HDF5(FILE="",NAME="",N1=,N2=,N3=,N4=,N5=,N6=,N7=)
  ! 
  !   Read the dimensions of an array in an HDF5 file.
  !=========================================================================

  SUBROUTINE getdim_hdf5 (file,name,N1,N2,N3,N4,N5,N6,N7)
    ...
    CALL H5DGET_SPACE_F(DATASET_ID=dset_id,DATASPACE_ID=dspace_ID,HDFERR=error)
    CALL H5SGET_SIMPLE_EXTENT_NDIMS_F(SPACE_ID=dspace_ID,RANK=rank, &
                                         HDFERR=error)
    CALL H5SGET_SIMPLE_EXTENT_DIMS_F(SPACE_ID=dspace_ID,DIMS=dims(1:rank), &
                                        MAXDIMS=maxdims(1:rank),HDFERR=error)
  ...
  !==========================================================================
  ! CALL READCOL (file,vec1,vec2,vec3,vec4,vec5,)
  !
  !   Reads column ASCII files.
  !==========================================================================

  ! Improvements: 
  !  - allow different column formats (str, int, float)
  !  - add a comment character
  !  - add a delimiter                
  SUBROUTINE readcol (file,vec1,vec2,vec3,vec4,vec5)
    ...
    IF (PRESENT(vec5)) THEN
      N = 5
    ELSE IF (PRESENT(vec4)) THEN
      N = 4
    ELSE IF (PRESENT(vec3)) THEN
      N = 3
    ELSE IF (PRESENT(vec2)) THEN
      N= 2
    ELSE
      N = 1
    END IF
    ...
    DO
      SELECT CASE (N)
        CASE (1)
          READ(unitdata,*,IOSTAT=iostat) v1
        CASE (2)
          READ(unitdata,*,IOSTAT=iostat) v1, v2
        CASE (3)
          READ(unitdata,*,IOSTAT=iostat) v1, v2, v3
        CASE (4)
          READ(unitdata,*,IOSTAT=iostat) v1, v2, v3, v4
        CASE (5)
          READ(unitdata,*,IOSTAT=iostat) v1, v2, v3, v4, v5
      END SELECT
      IF (iostat /= 0) EXIT
      CALL INCRARR(vec1,v1)
      IF (N >= 2) CALL INCRARR(vec2,v2)
      IF (N >= 3) CALL INCRARR(vec3,v3)
      IF (N >= 4) CALL INCRARR(vec4,v4)
      IF (N >= 5) CALL INCRARR(vec5,v5)
    END DO
    CLOSE(unitdata)
