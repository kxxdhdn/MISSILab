...
PROGRAM presimu_an
  ...
  !! Input variables
  INTEGER :: i, j, x, y, iw, ical, ipar, it
  ...
  chi2OK = .FALSE. ! see quick analysis in presimu_chi2
  IF (chi2OK) THEN
  ...
  DO j=1,2
    IF (j==1) THEN
      fileHB = TRIMLR(dirOUT)//'presimu_bb'//h5ext
    ...
    IF (hbOK) THEN
      IF (j==1) THEN
        PRINT*, 'HISTOIRE: presimu_hb BB analysis'
      ELSE IF (j==2) THEN
        PRINT*, 'HISTOIRE: presimu_hb HB analysis'
      END IF
