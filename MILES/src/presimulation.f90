!******************************************************************************
!*
!*                 GENERATE SIMULATION PARAMETERS (BY CHI2 FIT)
!*
!******************************************************************************


PROGRAM presimulation
  ...
  !! Output settings
  filOUT = TRIMLR(dirOUT)//'presimulation'//h5ext
  
  fiLOG = TRIMLR(dirOUT)//'log_presimulation'//ascext
  ...
  par(:,:,:) = parini(:,:,:,1)
  CALL WRITE_HDF5(STRARR1D=parinfo(:)%name, NAME='Best fitted parameter label', &
    ...
    chi2fity: DO yOBS=1,Ny
      notmasked: IF (ANY( mask(xOBS,yOBS,:) )) THEN
        ...
        ELSE
          ...
          DO j=1,NiniMC
            DO i=1,MERGE(2,1,verbose)
              WRITE(unitlog(i),*) 'iniMC iteration: '//TRIMLR(PRING(j))//'/'// &
                                  TRIMLR(PRING(NiniMC))
            END DO
          ...
          DO i=1,MERGE(2,1,verbose)
            WRITE(unitlog(i),*) "  MC chi2red is between " &
              ...
              //TRIMLR(PRING(MAXVAL(chi2redMC(:)),NDEC=6))//NEW_LINE('')
          END DO
        ...
        DO i=1,MERGE(2,1,verbose)
          WRITE(unitlog(i),*) '>>>'
          WRITE(unitlog(i),*) "Checking output ("//TRIMLR(PRING(xOBS))//', '// &
                              TRIMLR(PRING(yOBS))//'): '
  ...
  DO i=1,MERGE(2,1,verbose)
    WRITE(unitlog(i),*)
    WRITE(unitlog(i),*) 'LE MIROIR: Chi2 fit of galgen (M83 via IRS) [done]'
    WRITE(unitlog(i),*) REPEAT('=',textwid)//NEW_LINE('')
  END DO
  ...
  DO i=1,MERGE(2,1,verbose)
    WRITE(unitlog(i),*)
    WRITE(unitlog(i),*) 'LE MIROIR: presimulation analysis [done]'
    WRITE(unitlog(i),*) REPEAT('=',textwid)//NEW_LINE('')
  END DO
  ...
  !! Free memory space
  DEALLOCATE(wOBS, FnuOBS, dFnuOBS, maskint, mask, FnuMOD)

END PROGRAM presimulation
