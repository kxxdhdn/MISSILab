#!/usr/bin/env python
# -*- coding: utf-8 -*-

"""

This is the visualization of galspec.h5 (simulated spectra)

"""
import os
import numpy as np
import matplotlib.pyplot as plt

## astylo
from astylo.iolib import read_hdf5
from astylo.plib import plot2d_m

title = 'Simulated Spectra (rest frame)'

col_tab = ['darkred','r','pink',
           # 'orange','gold','y',
           'darkgreen','g','lime',
           'darkblue','b','cyan']

## Path
mroot = os.path.dirname(os.path.abspath(__file__))+'/../out1/'
path_fig = mroot+'Figures/'
if not os.path.exists(path_fig):
    os.makedirs(path_fig)
filobs = mroot+'galspec'

## Read h5 file
spec_unit = read_hdf5(filobs, 'Spectral unit')[0]
wvl = read_hdf5(filobs, 'Wavelength (microns)')
FnuOBS = read_hdf5(filobs, 'FnuOBS ('+spec_unit+')')
dFnuOBS = read_hdf5(filobs, 'dFnuOBS ('+spec_unit+')')
FnuBAND = read_hdf5(filobs, 'FnuBAND ('+spec_unit+')')
Nw, Ny, Nx = FnuOBS.shape
Nband = FnuBAND.shape[0]

## Make plots
xlab = r'$\lambda\,(\mu m)$'
if spec_unit=='MKS':
    ylab = r'Surface brightness $\,(W/m2/Hz/sr)$'
elif spec_unit=='MJyovsr':
    ylab=r'Surface brightness $\,(MJy/sr)$'

## Create data tables
lab_tab = np.empty((Nx,Ny), dtype=('<U30'))
Fnu_tab = []
dFnu_tab = []
for x in range(Nx):
    for y in range(Ny):
        lab_tab[x,y] = '('+str(x+1)+','+str(y+1)+')'
        Fnu_tab.append(FnuOBS[:,y,x])
        dFnu_tab.append(dFnuOBS[:,y,x])

Fnu_tab = np.array(Fnu_tab).reshape((Nx,Ny,Nw))
dFnu_tab = np.array(dFnu_tab).reshape((Nx,Ny,Nw))

 ## Plot individual spectrum with S/N
##------------------------------------
for y in range(Ny):
    for x in range(Nx):
        filename = path_fig+'galspec_'+lab_tab[x,y]+'.png'
        
        p = plot2d_m(wvl, [Fnu_tab[x,y,:]/dFnu_tab[x,y,:], Fnu_tab[x,y,:], dFnu_tab[x,y,:]],
                     yerr=[0.,dFnu_tab[x,y,:],0.],
                     xall=wvl, xlog=0, ylog=0, xlim=(4.5, 22.), ylim=(0,1.e3), 
                     # xall=wvl, 
                     title=title, xlab=xlab, ylab=ylab+'/SN ratio', 
                     lablist=['S/N', 'Data', 'Unc'], cl=['y', 'k', 'r'], 
                     legend='best', figsize=(10,6))
        
        p.save(filename)

## Plot stacked spectra
##----------------------
for x in range(Nx):
    filename = path_fig+'galspec_stack_x='+str(x+1)+'.png'
    
    p = plot2d_m(wvl, Fnu_tab[x,:,:], #yerr=dFnu_tab[x,:,:], 
                 xall=wvl, xlog=0, ylog=0, xlim=(4.5, 22.), ylim=(0.,1.e3), 
                 title=title, xlab=xlab, ylab=ylab, cl=col_tab,
                 lablist=lab_tab[x,:], legend='best', figsize=(10,6))
    
    p.save(filename)

## Plot individual band in table
##-------------------------------
for x in range(Nx):
    filename = path_fig+'galspec_table_x='+str(x+1)+'.png'

    fig, axes = plt.subplots(nrows=3, ncols=3, figsize=(9,9))
    plt.subplots_adjust(left=.1, bottom=.05, \
                        right=.99, top=.95, wspace=.3, hspace=.4)
    for y in range(Ny):
        px, py = int(y/3), y%3
        axes[px,py].plot(wvl, Fnu_tab[x,y,:],c='k')
        for i in range(2):
            i += 4
            axes[px,py].plot(wvl, FnuBAND[i,:,y,x],c='b')
        axes[px,py].set_xlim((6., 6.4))
        axes[px,py].set_ylim((0, 2.e2))
        axes[px,py].set_xlabel(xlab)
        axes[px,py].set_ylabel(ylab)
        axes[px,py].set_title(
            fr'$Spectra ({x+1},{y+1})$')

    plt.savefig(filename)
    
# plt.show()

print('>>> Coucou show_galspec [Done] <<<')
