...
CONTAINS
  ...
  !!-------------------------------------------------------
  !!
  !! Find par index of the reference band intensity to calculated ratios
  !!
  !!-------------------------------------------------------
  SUBROUTINE set_indref( indref, namref, parinfo )

    USE datable, ONLY: TABand
    USE utilitties, ONLY: trimeq
    USE arrays, ONLY: iwhere
    ...
    CALL IWHERE( TRIMEQ(TABand(:)%label,namref), itab ) ! itab = ind of TABand
    CALL IWHERE( TRIMEQ(labB(:), 'l'), indref ) ! ilab = ind of labB
  ...
  FUNCTION specModel_3D( wvl, indpar, parval, Qabs, extinct, verbose, &
    ...
    FORALL (x=1:Nx,y=1:Ny,i=1:Nline) &
      dblarr4d(x,y,:,i) = EXP(parval(x,y,indpar%lnRline(i))) * &
  ...
  FUNCTION specModel_gen( wvl, parvec, parname, parinfo, indpar, parval, &
    ...
    sampLINE: IF (gridLINE) THEN
      ...
      ELSE IF (gridCline) THEN
        READ(parname(6:7),*) j
        FORALL (igrid=1:Ngrid) &
          specModel_gen(igrid,:) = EXP(parval(indpar%lnRline(j))) &
      ...
      DO i=1,Nline
        IF (i /= j) &
          spec1D(:) = spec1D(:) + EXP(parval(indpar%lnRline(i))) &
