...
spec_name = []
for i in range(Nobs):
    xscale, yscale = read_hdf5(
        croot+'../data/build_history_'+namobs[i], 'Super pixel size')
    fits_obs = croot+'../data/M82_'+namobs[i] # obs
    ...
    for y in range(Ny):
        ...
        if y%yscale==0:
            spec_name.append(namobs[i][0]+str(ys+1))
            d = ds.data[:,y,0][:,np.newaxis,np.newaxis]
            e = ds.unc[:,y,0][:,np.newaxis,np.newaxis]
            if (i==0 and y==0):
                data0 = d
                unc0 = e
                wave0 = ds.wave
            else:
                data0 = np.append(data0, d, axis=1)
                unc0 = np.append(unc0, e, axis=1)
spec_name = np.array(spec_name)[:,np.newaxis]
...
dictune = [ dict([ ('name','default'),
            ...
            dict([ ('namall','lnLstar'),('fixed','F'),('hyper','T'),]),
            ...
            #        ('fixed','F'),('hyper','T') ]),
            # ## Small 3.5
            # dict([ ('name','Cband'+str(labB.index('Small 3.5    ')+1)),
            #        ('fixed','F'),('hyper','T') ]),
            # dict([ ('name','WSband'+str(labB.index('Small 3.5    ')+1)),
            #        ('fixed','F'),('hyper','T') ]),
            # dict([ ('name','WLband'+str(labB.index('Small 3.5    ')+1)),
            #        ('fixed','F'),('hyper','T') ]),
            # ## Main 6.2 (1)
            # dict([ ('name','Cband'+str(labB.index('Main 6.2 (1) ')+1)),
            #        ('fixed','F'),('hyper','T') ]),
            # ## Main 6.2 (2)
            # dict([ ('name','Cband'+str(labB.index('Main 6.2 (2) ')+1)),
            #        ('fixed','F'),('hyper','T') ]),
            # ## Plateau 7.7
            # dict([ ('name','Cband'+str(labB.index('Plateau 7.7  ')+1)),
            #        ('fixed','F'),('hyper','T') ]),
            # ## Main 7.7 (1)
            # dict([ ('name','Cband'+str(labB.index('Main 7.7 (1) ')+1)),
            #        ('fixed','F'),('hyper','T') ]),
            # ## Main 7.7 (2)
            # dict([ ('name','Cband'+str(labB.index('Main 7.7 (2) ')+1)),
            #        ('fixed','F'),('hyper','T') ]),
            # ## Main 8.6
            # dict([ ('name','Cband'+str(labB.index('Main 8.6     ')+1)),
            #        ('fixed','F'),('hyper','T') ]),
            # ## Small 11.0
            # dict([ ('name','Cband'+str(labB.index('Small 11.0  ')+1)),
            #        ('fixed','F'),('hyper','T') ]),
            # ## Main 11.2
            # dict([ ('name','Cband'+str(labB.index('Main 11.2    ')+1)),
            #        ('fixed','F'),('hyper','T') ]),
            # ## Plateau 11.3
            # dict([ ('name','Cband'+str(labB.index('Plateau 11.3 ')+1)),
            #        ('fixed','F'),('hyper','T') ]),
            # ## Main 12.7 (1)
            # dict([ ('name','Cband'+str(labB.index('Main 12.7 (1)')+1)),
            #        ('fixed','F'),('hyper','T') ]),
            # ## Main 12.7 (2)
            # dict([ ('name','Cband'+str(labB.index('Main 12.7 (2)')+1)),
            #        ('fixed','F'),('hyper','T') ]),
            # ## Plateau 17.0
            # dict([ ('name','Cband'+str(labB.index('Plateau 17.0 ')+1)),
            #        ('fixed','F'),('hyper','T') ]),
...
write_hdf5(h5_obs, 'Initial parameter label', ds.name, append=True, verbose=noisy)
if (chi2init):
    parini = read_hdf5(h5_chi2, 'Best fitted parameter value')
    write_hdf5(h5_obs, 'Initial parameter value', parini, append=True, verbose=True)
