...
PROGRAM genpar
  ...
  USE chi2_kit, ONLY: Nx, Ny, NwOBS, wOBS, nuOBS, FnuOBS, dFnuOBS, &
  ...
  CHARACTER(lenpath) :: dirOUT, filOUT, fiLOG
  ...
  !! Output settings
  !!-----------------
  filOUT = TRIMLR(dirOUT)//'pargen'//h5ext
  
  fiLOG = TRIMLR(dirOUT)//'log_genpar'//ascext
  ...
  DO i=1,MERGE(2,1,verbose)
    WRITE(unitlog(i),*)
    WRITE(unitlog(i),*) 'Chi2 fit galgen (M83 via IRS) [done]'
    WRITE(unitlog(i),*) REPEAT('=',textwid)//NEW_LINE('')
  END DO
  ...
  DO i=1,MERGE(2,1,verbose)
    WRITE(unitlog(i),*)
    WRITE(unitlog(i),*) 'Show fit galgen [done]'
    WRITE(unitlog(i),*) REPEAT('=',textwid)//NEW_LINE('')
  END DO
