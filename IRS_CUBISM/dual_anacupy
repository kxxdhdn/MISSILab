...
from calipy import synthetic_photometry as calib
...
def bispecview(wvl, data1, data2, wfilt, filt_width, Fnu_filt):
	"""
	extract spectrum at chosen square

	--- INPUT ---
	wvl           wavelength
	data1         box (NAXIS3, npix) ==> not(b0==2)
	data2         box in ref frame
	"""
	fig2 = plt.figure("box spectrum")
	ax2 = plt.subplot()
	ax2.errorbar(wvl, np.nansum(data1, axis=1), 0.,
		c='k', ls='-', lw=.5, ecolor='r', elinewidth=1., label="data")
	ax2.errorbar(wvl, np.nansum(data2, axis=1), 0.,
		c='b', ls='-', lw=.5, ecolor='r', elinewidth=1., label="ref")
	## add inter-calibration
	ax2.errorbar(wfilt, Fnu_filt, xerr=filt_width,
		fmt='o', capsize=5, c='g', ecolor='g', elinewidth=.5, label="photometry")
...
filename0 = '1_LL1_cube' 
calib_path = '/Users/dhu/data/mosaic/SMC/'
calib_file = 'mips024'
...
hdul0 = fits.open(path+filename0+'.fits')
data0 = hdul0[0].data
hdul_c = fits.open(calib_path+calib_file+'.fits')
dcalib = hdul_c[0].data
...
while True:
	...
	if b1=="0":
		...
		if not(b0=="2"):
			...
			## fluxes in box 
			data1 = data[:, yb:yt, xb:xt].reshape((NAXIS3, (xt-xb)*(yt-yb))) # (NAXIS3, npix)
			data2 = data0[:, yb:yt, xb:xt].reshape((NAXIS3, (xt-xb)*(yt-yb)))
		elif b0=="2":
			...
			for dx in range(radius+1):
				for dy in range(radius+1):
					...
					if (dx==0 and dy==0):
						...
						data2.append(data0[:, y0+dy, x0-dx])
						data2.append(data0[:, y0+dy, x0+dx])
			data1 = np.array(data1).transpose(1,0) # (npix, NAXIS3) -> (NAXIS3, npix)
			data2 = np.array(data2).transpose(1,0)
			print("(nb of pix in circle, NAXIS3) = ", data1.shape)

		## view spectrum
		flux0 = np.nansum(data1, axis=1).reshape((NAXIS3,1,1))
		wcen, Fnu_filt = calib(wvl, flux0, ["MIPS1"])
		print("d", wcen, Fnu_filt)
		bispecview(wvl, data1, data2, wcen[0], 3.2, Fnu_filt[0,0,0])
